...
import com.crm.service.SubPipelineQueryService;
...
/**
 * REST controller for managing {@link com.crm.domain.SubPipeline}.
 */
@RestController
@RequestMapping("/api/sub-pipelines")
public class SubPipelineResource {

    private final Logger log = LoggerFactory.getLogger(SubPipelineResource.class);
    ...
    public ResponseEntity<SubPipelineDTO> createSubPipeline(@Valid @RequestBody SubPipelineDTO subPipelineDTO) throws URISyntaxException {
        ...
        SubPipelineDTO result = subPipelineService.save(subPipelineDTO);
        return ResponseEntity
            .created(new URI("/api/sub-pipelines/" + result.getId()))
    ...
    public ResponseEntity<SubPipelineDTO> updateSubPipeline(
        ...
        SubPipelineDTO result = subPipelineService.update(subPipelineDTO);
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, subPipelineDTO.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<SubPipelineDTO> partialUpdateSubPipeline(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, subPipelineDTO.getId().toString())
        );
    }
