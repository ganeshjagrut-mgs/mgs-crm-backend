...
public class CustomerRepositoryWithBagRelationshipsImpl implements CustomerRepositoryWithBagRelationships {
    ...
    public Optional<Customer> fetchBagRelationships(Optional<Customer> customer) {
        return customer.map(this::fetchMasterCategories);
    }
    ...
    public List<Customer> fetchBagRelationships(List<Customer> customers) {
        return Optional.of(customers).map(this::fetchMasterCategories).orElse(Collections.emptyList());
    }

    Customer fetchMasterCategories(Customer result) {
        return entityManager
            .createQuery(
                "select customer from Customer customer left join fetch customer.masterCategories where customer.id = :id",
                Customer.class
            )
    ...
    List<Customer> fetchMasterCategories(List<Customer> customers) {
        ...
        List<Customer> result = entityManager
            .createQuery(
                "select customer from Customer customer left join fetch customer.masterCategories where customer in :customers",
                Customer.class
            )
