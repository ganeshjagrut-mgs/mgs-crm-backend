...
public interface CityRepository extends JpaRepository<City, Long>, JpaSpecificationExecutor<City> {
    ...
    @Query(value = "select city from City city left join fetch city.state", countQuery = "select count(city) from City city")
    Page<City> findAllWithToOneRelationships(Pageable pageable);

    @Query("select city from City city left join fetch city.state")
    List<City> findAllWithToOneRelationships();

    @Query("select city from City city left join fetch city.state where city.id =:id")
