  ...
  return (
    <div>
      <h2 id="state-heading" data-cy="StateHeading">
        <Translate contentKey="crmApp.state.home.title">States</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="crmApp.state.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/state/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="crmApp.state.home.createLabel">Create new State</Translate>
          </Link>
        </div>
      </h2>
        ...
        {stateList && stateList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="crmApp.state.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('name')}>
                  <Translate contentKey="crmApp.state.name">Name</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('name')} />
                </th>
                <th className="hand" onClick={sort('code')}>
                  <Translate contentKey="crmApp.state.code">Code</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('code')} />
                </th>
                ...
                <th className="hand" onClick={sort('active')}>
                  <Translate contentKey="crmApp.state.active">Active</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('active')} />
                </th>
                <th>
                  <Translate contentKey="crmApp.state.country">Country</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {stateList.map((state, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{state.name}</td>
                  <td>{state.code}</td>
                  <td>{state.createdAt ? <TextFormat type="date" value={state.createdAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{state.createdBy}</td>
                  <td>{state.updatedAt ? <TextFormat type="date" value={state.updatedAt} format={APP_DATE_FORMAT} /> : null}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="crmApp.state.home.notFound">No States found</Translate>
            </div>
          )
        )}
      </div>
